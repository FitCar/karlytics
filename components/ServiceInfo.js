...
const ServiceInfo = ({ current_car }) => {
  ...
  return (
    <View>
      <Text style={tw`font-semibold text-gray-500 text-lg mb-1`}>
        Car Servicing
      </Text>
      ...
      {progressLeft <= 1 ? (
        <View style={tw`flex-row justify-between`}>
          <Text style={tw`font-semibold text-gray-600`}>
            Due by {current_car?.nextServiceDate}
          </Text>
          <Text style={tw`font-semibold text-gray-600`}>{duration} left</Text>
        </View>
      ) : (
        <Text style={tw`font-medium text-red-500 text-sm mb-2`}>
          Due For Servicing
        </Text>
      )}
      ...
      <View style={tw`my-1`}>
        <ProgressBar
          ...
          color={progressLeft > 1 ? "red" : "#2bced6"}
        />
      </View>
    </View>
  );
